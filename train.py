...
def evaluate(dataset, model, args, name='Validation', max_num_examples=None):
    ...
    print(name, " F1~:", result['F1'])
    return result
def calculate_explanation_accuracy(model, dataset):
    model.eval()
    ...
    for data in dataset:
        ...
        batch_num_nodes = data['num_nodes'].int().numpy()
        assign_input = Variable(data['assign_feats'].float(), requires_grad=False).cuda()
        pred , _ , _= model(h0, adj, batch_num_nodes, assign_x=assign_input)
        confidence = F.softmax(pred, dim=1).max(dim=1)[0]
        confidence_diff = confidence.mean().item()
        total_confidence_diff += confidence_diff
        num_samples += 1
    explanation_accuracy = total_confidence_diff / num_samples
    ...
    if assign_tensor is not None:
        ...
        for i in range(len(batch_idx)):
            ...
            if len(node_colors) != num_nodes:
                print(f"节点数量: {num_nodes}, 颜色数量: {len(node_colors)}")
                if len(node_colors) < num_nodes:
                    node_colors = np.resize(node_colors, num_nodes)
                else:
...
import matplotlib.pyplot as plt
from torch.autograd import Variable
def save_graph_picture(matrix, layer_name, output_dir, batch_idx, prefix, is_adj=False):
    ...
    if is_adj:
        ...
        plt.colorbar()
    plt.title(f"{prefix}_{layer_name}")
    plt.savefig(os.path.join(output_dir, f'{prefix}_{layer_name}_batch{batch_idx}.png'))
    plt.close()
def forward_and_print_shapes(dataset, model, mask_nodes, output_dir="pic"):
    sample_data = next(iter(dataset))
    adj = Variable(sample_data['adj'].float(), requires_grad=False).cuda()
    h0 = Variable(sample_data['feats'].float(), requires_grad=False).cuda()
    batch_num_nodes = sample_data['num_nodes'].int().numpy() if mask_nodes else None
    assign_input = Variable(sample_data['assign_feats'].float(), requires_grad=False).cuda()
    layer_idx = 0
    def hook(module, input, output):
        nonlocal layer_idx
        input_shapes = [inp.shape for inp in input] if isinstance(input, tuple) else input.shape
        output_shapes = [out.shape for out in output] if isinstance(output, tuple) else output.shape
        if isinstance(input, tuple):
            for idx, inp in enumerate(input):
                if isinstance(inp, torch.Tensor):
                    save_graph_picture(inp, module.__class__.__name__, output_dir, batch_idx=0, prefix=f"layer{layer_idx}_input{idx}", is_adj=(idx == 1 and module.__class__.__name__ == 'GraphConv'))
        elif isinstance(input, torch.Tensor):
            save_graph_picture(input, module.__class__.__name__, output_dir, batch_idx=0, prefix=f"layer{layer_idx}", is_adj=False)
        if isinstance(output, torch.Tensor):
            save_graph_picture(output, module.__class__.__name__, output_dir, batch_idx=0, prefix=f"layer{layer_idx}_output", is_adj=(module.__class__.__name__ == 'GraphConv'))
        layer_idx += 1
    hooks = []
...
def train(dataset, model, args, same_feat=True, val_dataset=None, test_dataset=None, writer=None,
        mask_nodes=True, explainable_model=None):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, data in enumerate(dataset):
            ...
            assign_input = Variable(data['assign_feats'].float(), requires_grad=False).cuda()
            ypred, perturb_reg , route_probs= model(h0, adj, batch_num_nodes, assign_x=assign_input, current_epoch=epoch, total_epochs=args.num_epochs)
            if not args.method == 'soft-assign' or not args.linkpred:
                loss = model.loss(ypred, label, perturb_reg=perturb_reg, route_probs=route_probs, alpha=0.1)
            else:
                loss = model.loss(ypred, label, adj, batch_num_nodes, perturb_reg=perturb_reg, route_probs=route_probs, alpha=0.1)
            loss.backward()
        ...
        if val_dataset is not None:
            ...
            explanation_accuracy = calculate_explanation_accuracy(model, val_dataset)
            print(f"Epoch {epoch} Validation Explanation Accuracy: {explanation_accuracy}")
            if val_result['acc'] > best_val_result['acc']:
                best_val_result['acc'] = val_result['acc']
                best_val_result['epoch'] = epoch
                best_val_result['loss'] = avg_loss
                best_model = model.state_dict()
        if test_dataset is not None:
            explanation_accuracy_test = calculate_explanation_accuracy(model, test_dataset)
            print(f"Epoch {epoch} Test Explanation Accuracy: {explanation_accuracy_test}")
        if writer is not None:
    ...
    if test_dataset is not None:
        ...
        explanation_accuracy = calculate_explanation_accuracy(model, explainable_model, test_dataset)
    ...
    save_path = gen_train_plt_name(args)
...
def prepare_data(graphs, args, test_graphs=None, max_nodes=0):
    ...
    test_dataset_loader = torch.utils.data.DataLoader(
            ...
            shuffle=False,
            num_workers=args.num_workers)
    return train_dataset_loader, val_dataset_loader, test_dataset_loader,            dataset_sampler.max_num_nodes, dataset_sampler.feat_dim, dataset_sampler.assign_feat_dim
...
def benchmark_task(args, writer=None, feat='node-label'):
    ...
    if feat == 'node-feat' and 'feat_dim' in graphs[0].graph:
        ...
        for G in graphs:
            featgen_const.gen_node_features(G)
    train_dataset, val_dataset, test_dataset, max_num_nodes, input_dim, assign_input_dim =            prepare_data(graphs, args, max_nodes=args.max_nodes)
    if args.method == 'soft-assign':
        print('Method: soft-assign')
        model = encoders.SoftPoolingGcnEncoder(
                max_num_nodes,
